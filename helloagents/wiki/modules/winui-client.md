...
- **特性:** Chat 执行过程中展示 diff（自动展开 Trace；`+`/`-` 行带半透明颜色背景），结束时输出变更汇总（按文件净变化）
- **状态:** 开发中
- **最后更新:** 2026-01-23
...
侧边栏会话项右侧展示会话状态：运行中显示 ProgressRing；完成且不在该会话显示绿点；异常显示黄点；进入该会话后清除点位。
...
设置页提供“自动翻译（Trace）”配置：用于配置 Bridge Server 的 `Bridge:Translation`（全局开关、OpenAI 兼容 BaseUrl+ApiKey、Model、每秒最大请求数（默认 1）与最大并发（默认 2））；应用后会重启本机后端，目标语言固定为 `zh-CN`。
...
- 局域网地址自动选择：默认使用首选局域网 IPv4（优先私网地址、带默认网关的真实网卡，尽量避开虚拟网卡/测试网段），并以只读文本显示，无需手动下拉选择
